...
def audio_load(pathname, filename):
    song = AudioSegment.from_file(Path(pathname)/ filename)
...
def block(vggish_features, window, hop):
    ...
    index_hop = np.arange(0,vggish_features.shape[0] - hop - (vggish_features.shape[0]%window), hop)[:, np.newaxis]
